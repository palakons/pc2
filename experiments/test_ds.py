...
def plot_annotation(img_rgb,radar_pcl,output_fname=None):

    gs = gridspec.GridSpec(2, 2)
    fig = plt.figure(   figsize=(10, 10))
    # plt.get_current_fig_manager().full_screen_toggle()

    # plot radar pcl on front view
    ax = fig.add_subplot(gs[0, 0])
    ax.scatter(radar_pcl[:, 1], radar_pcl[:, 2], c='darkblue', s=1, alpha=0.5)
    ax.set_title('Radar Point Cloud Front view')
    ax.set_ylim(-10, 10)
    ax.set_xlim(-50, 50)
    ax.set_xlabel('dim 1')  
    ax.set_ylabel('dim 2')
    ...
    ax.scatter(radar_pcl[:, 1], radar_pcl[:, 0], c='darkblue', s=1, alpha=0.5)
    ax.set_title('Radar Point Cloud Top view:')
    ax.set_ylim(-10, 100)
    ax.set_xlim(-50, 50)
    ax.set_xlabel('dim 1')  
    ax.set_ylabel('dim 0')
    ...
    #make image from tensor img_rgb
    camera_image =img_rgb
